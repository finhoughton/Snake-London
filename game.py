...
class GameState(GameContext):
    ...
    def neck(self, team: Team) -> list[str]:
        ...
        if snake.travel_line is None:
            raise GameError(f"{team!r} has no declared line")
    ...
    def _acting_snake(self, team: Team) -> Snake:
        ...
        snake = self.get_snake(team)
        if snake.crashed:
            raise GameError(f"{team!r} has crashed and can no longer act")
        if snake.conceded:
            raise GameError(f"{team!r} has conceded and can no longer act")
        return snake
    ...
    def initial_request_challenge(self, team: Team) -> None:
        ...
        if snake.travel_line is not None:
            raise GameError(f"{team!r} has already completed their initial challenge")
        if snake.neck_active:
            raise GameError(f"{team!r} already has an active challenge request")
    ...
    def request_challenge(self, team_id: int, station: str) -> None:
        ...
        if snake.travel_line is None:
            raise GameError(f"{team!r} has no declared line — use initial_request_challenge() first")
        if not self.map.has_station(station):
            raise GameError(f"Unknown station: {station!r}")
        if not self.map.get_station(station).has_line(snake.travel_line):
            raise GameError(f"{station!r} is not on line {snake.travel_line!r}")
        if station == snake.anchor:
            raise GameError(f"{station!r} is the current Anchor — travel to a different interchange")
        if snake.neck_active:
            raise GameError(f"{team!r} already has an active challenge request")
        if snake.blocked_station is not None and station == snake.blocked_station:
            raise GameError(f"{station!r} was just retreated from — request a different interchange")
        if snake.vetoed:
            raise GameError(f"{team!r} is in their veto period")
    ...
    @event()
    def complete_challenge(self, team_id: int, next_line: str, *, hard: bool = False) -> tuple[list[str], list[Team]]:
        ...
        Returns the list of newly claimed interchanges and the list of newly crashed teams.
        """
        team = self.get_team(team_id)
        snake = self._acting_snake(team)
        if self.status != Status.RUNNING:
        ...
        if not snake.neck_active:
            raise GameError(f"{team!r} has no active challenge request")
        is_initial = snake.travel_line is None
        if not self.map.has_line(next_line):
            raise GameError(f"Unknown line: {next_line!r}")
        if not self.map.get_station(snake.front).has_line(next_line):
            raise GameError(f"Front interchange {snake.front!r} is not on line {next_line!r}")
        if snake.vetoed:
            raise GameError(f"{team!r} is in their veto period")
        ...
        crashed_teams = self._apply_neck_crashes(exclude=team)
        ...
        return newly_claimed, crashed_teams
    ...
    def veto_challenges(self, team_id: int) -> bool:
        ...
        if self.status != Status.RUNNING:
            raise GameError("The game is not running")
        if not snake.neck_active:
            raise GameError(f"{team!r} has no active challenge to veto")
        if snake.vetoed:
            raise GameError(f"{team!r} is in their veto period")
    ...
    def buy_powerup(self, team_id: int, powerup_id: str) -> list[Curse] | None:
        ...
        Raises GameError if the team is out of the game, the id is unknown, the
        ...
        if powerup_id not in POWERUP_COSTS:
            raise GameError(f"Unknown powerup: {powerup_id!r}")
        if powerup_id not in self.enabled_powerups:
            raise GameError(f"Powerup {powerup_id!r} is not enabled in this game")
        cost = POWERUP_COSTS[powerup_id]
        if snake.coins < cost:
            raise GameError(f"Not enough coins to buy {powerup_id!r}: need {cost}, have {snake.coins}")
    ...
    def play_normal_powerup(self, team_id: int, powerup_id: str) -> None:
        ...
        failed play (the handler raises GameError on bad input) keeps the card.
        """
        team = self.get_team(team_id)
        snake = self._acting_snake(team)
        if self.status != Status.RUNNING:
        ...
        if powerup_id not in snake.hand:
            raise GameError(f"{powerup_id!r} is not in {team!r}'s hand")
    ...
    def choose_curse(self, team_id: int, curse_id: str) -> Curse:
        ...
        if kept is None:
            raise GameError(f"{team!r} was not offered a curse with id {curse_id!r}")
    ...
    def play_curse(self, team_id: int, target_team_id: int, curse_id: str) -> Curse:
        ...
        if "curse" not in snake.hand:
            raise GameError(f"curse is not in {team!r}'s hand")
        if curse_id not in [c.id for c in snake.held_curses]:
            raise GameError(f"{curse_id} is not in {team!r}'s hand")
    ...
    def play_jump(self, team_id: int, station: str) -> None:
        ...
        if "jump" not in snake.hand:
            raise GameError(f"jump is not in {team!r}'s hand")
    ...
    def play_detour(self, team_id: int, line: str) -> None:
        ...
        if "detour" not in snake.hand:
            raise GameError(f"detour is not in {team!r}'s hand")
    ...
    def concede(self, team: Team) -> None:
        ...
        snake = self.get_snake(team)
        if snake.eliminated:
            raise GameError(f"{team!r} is already out of the game")
        snake.conceded = True
    ...
    def _apply_neck_crashes(self, exclude: Team) -> list[Team]:
        ...
        *become* claimed by another team crashes that snake (the primary lose
        ...
        for other_team, other_snake in self.snakes.items():
            ...
            if not self.is_neck_safe(other_team):
                crashed_teams.append(other_team)
                self.crash(other_team)
        
        return crashed_teams
    ...
    async def new_objective_callback(self, station: str | None) -> None:
    ...
    def new_objective(self) -> str | None:
        """Place a new objective and schedule the next."""
        if self.status != Status.RUNNING:
            return None
    ...
    def declare_win(self, team_id: int) -> None:
        ...
        Announcing it is the bot's job. The check is scheduled here so it can't be forgotten.
        """
        if self.status != Status.RUNNING:
            return
        ...
        if snake.win_declared:
            raise GameError(f"{team!r} has already declared a win")
        if snake.declare_cooldown:
            raise GameError(f"{team!r} declared recently and failed; wait for the cooldown to end")
        if snake.coins < DECLARE_WIN_COST:
            raise GameError(f"Not enough coins to declare a win: need {DECLARE_WIN_COST}, have {snake.coins}")
    ...
    async def win_declaration_callback(self, ret: tuple[bool, int]):
        won, team_id = ret
    ...
    def resolve_win_declaration(self, team_id: int) -> tuple[bool, int]:
        ...
        if self.status != Status.RUNNING:
            return False, team_id  # the game already ended some other way
    ...
    async def declare_cooldown_end_callback(self, team: Team) -> None:
        if team.thread: await team.thread.send("Your declaration cooldown has ended!")

    @event(callback=declare_cooldown_end_callback)
    def end_declare_cooldown(self, team_id: int) -> Team:
    ...
    def _curse_by_id(self, curse_id: str) -> Curse:
        if self.curse_deck is None:
            raise GameError("This game has no curses")
        try:
            return self.curse_deck.get(curse_id)
        except KeyError:
            raise GameError(f"Unknown curse: {curse_id!r}") from None
    ...
    def admin_set_coins(self, team_id: int, coins: int) -> None:
        ...
        if coins < 0:
            raise GameError("Coins can't go below 0")
        snake.coins = coins
    ...
    def admin_give(self, team_id: int, powerup_id: str, curse_id: str = "") -> None:
        ...
        if (powerup_id == "curse") != bool(curse_id):
            raise GameError("Say which curse to give" if not curse_id else "Only a curse takes a curse_id")
        if not curse_id:
            snake.hand.append(powerup_id)
            return
    ...
    def admin_take(self, team_id: int, powerup_id: str, curse_id: str = "") -> None:
        ...
        snake = self.get_snake(team)
        if powerup_id not in snake.hand:
            raise GameError(f"{team!r} has no {POWERUP_NAMES.get(powerup_id, powerup_id)} card")
        if (powerup_id == "curse") != bool(curse_id):
            raise GameError("Say which curse to take" if not curse_id else "Only a curse takes a curse_id")
        if curse_id:
            held = next((c for c in snake.held_curses if c.id == curse_id), None)
            if held is None:
                raise GameError(f"{team!r} isn't holding the curse {curse_id!r}")
    ...
    def admin_set_line(self, team_id: int, line: str) -> None:
        ...
        snake = self.get_snake(team)
        if snake.neck_active:
            raise GameError(f"{team!r} has a challenge on; cancel it first")
        if not self.map.has_line(line):
            raise GameError(f"Unknown line: {line!r}")
        if not self.map.get_station(snake.anchor).has_line(line):
            raise GameError(f"The {line} doesn't go through {snake.anchor}")
    ...
    def admin_cancel_challenge(self, team_id: int) -> None:
        ...
        snake = self.get_snake(team)
        if not snake.neck_active:
            raise GameError(f"{team!r} has no challenge on")
        if snake.travel_line is None:
            raise GameError(f"{team!r} is on their first challenge, which can't be cancelled")
    ...
    def admin_end_veto(self, team_id: int) -> None:
        ...
        snake = self.get_snake(team)
        if not snake.vetoed:
            raise GameError(f"{team!r} isn't in a veto period")
        snake.vetoed = False
    ...
    def admin_bring_back(self, team_id: int) -> None:
        ...
        if not snake.eliminated:
            raise GameError(f"{team!r} is still in the game")
    ...
    def admin_add_objective(self, station: str) -> None:
        ...
        if station in self.objectives:
            raise GameError(f"{station!r} is already an objective")
        self.objectives.append(station)
    ...
    def admin_remove_objective(self, station: str) -> None:
        ...
        if station not in self.objectives:
            raise GameError(f"{station!r} isn't a live objective")
    ...
    def __init__(
        ...
        if status in [jloxgame.Status.INIT, jloxgame.Status.SETUP]:
            self.team_positions_input = InputText(
                placeholder="Wembley Park, Abbey Wood",
        ...
        self.team_colors_input = InputText(
            placeholder=", ".join(DEFAULT_TEAM_COLORS[:2]), value=", ".join(team_colors), required=False
        )
